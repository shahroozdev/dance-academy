...
page("monthly-routine", "07 · MONTHLY BILLING", "Your monthly billing routine", "Follow this order before sending fees to parents.",
  ...
  note("Scheduled work", "Automatic bill generation and reminders depend on the deployment configuration. Confirm activation with your implementation team. Continue checking the month manually rather than assuming a scheduled job succeeded."));
...
page("parent-messages", "09 · PARENT COMMUNICATION", "Send a family fee notice", "Review the combined message and recipient before sending.",
  ...
  table(["Action", "What it does"], [["Copy Message", "Copies the preview for manual sending."], ["Open in WhatsApp", "Opens WhatsApp with the message prepared. Review and send it inside WhatsApp yourself."], ["Send Email", "Sends through the configured studio email service; shown when the family has an email address."], ["Send WhatsApp", "Uses the studio's configured WhatsApp service. Requires implementation-team activation and verified setup."], ["Mark as Sent", "Records a manually sent notice. It does not send a message by itself."], ["Send Payment Reminder", "Sends a reminder for eligible overdue, unpaid/partial bills, using the remaining balance."]]) +
  ...
  note("Delivery status", "A WhatsApp request accepted for delivery is not proof that the parent received or read it. Check the sending service if the result is uncertain before sending another copy."));
...
page("notifications", "09 · PARENT COMMUNICATION", "Review notifications and reminders", "Use the log to check what happened after a send attempt.",
  ...
  note("Scheduled reminders", "When scheduling is active, reminders use the due-day and reminder-delay settings, and target unpaid or partially paid bills. Successful scheduled reminders are sent once per bill; failed attempts can remain eligible for retry. Unfinalized class fees prevent sending.") +
...
page("billing-settings", "11 · OWNER SETTINGS", "Set discounts and reminder timing", "Use decimal fractions when entering discount percentages.",
  ...
  p("Reminder timing only results in automatic sending when the scheduled service is active and the necessary email/WhatsApp setup and fee finalization are complete."));
...
page("integrations", "11 · OWNER SETTINGS", "Manage email sending", "Use the connection details supplied by your email administrator.",
  ...
  note("Confirm service activation", "Ask the implementation team to verify sending with an agreed test recipient before relying on parent notices. The guide does not include passwords or secret access tokens.") +
  note("WhatsApp in this edition", "WhatsApp connection fields are not visible in the current Settings screen. Manual <b>Open in WhatsApp</b> is available in the family message window. Direct and scheduled WhatsApp sending require setup and verification by the implementation team."));
...
page("email-templates", "11 · OWNER SETTINGS", "Edit email wording", "Change the message while retaining the studio's standard email layout.",
  ...
  p("Read the saved message for tone, spelling, and clear payment instructions. These templates control email wording, not the manually prepared WhatsApp message or the WhatsApp provider's approved templates."));
...
page("checklists", "APPENDIX A", "Printable operating checklists", "Use these during handover and routine studio work.",
  ...
  h("Handover details") + table(["Item", "Studio record"], [["Admin website address", "________________________________________"], ["Parent registration address", "________________________________________"], ["Studio owner / administrator", "________________________________________"], ["Implementation support contact", "________________________________________"], ["Email sending verified on", "________________________________________"], ["Automatic billing / reminders confirmed", "________________________________________"], ["WhatsApp service activation confirmed", "________________________________________"]]) +
...
page("glossary", "APPENDIX B", "Glossary and status reference", "A shared vocabulary for studio administration.",
  ...
  note("Edition notes", "This manual describes the app reviewed on 08 September 2026. Screenshots show the current studio demonstration records and some empty starting screens. Amounts in worked examples are illustrative. Configured email account values are concealed. Service activation must be confirmed at handover."));
