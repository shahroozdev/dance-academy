...
WhatsApp template sending is implemented for protected monthly scheduling and optional WhatsApp payment reminders, including provider message references and failure logging. The direct send control is hidden while WhatsApp configuration is unavailable; staff can still use **Open in WhatsApp** for manual sending. Live API delivery remains unverified and requires working Meta credentials, approved templates, and deployment configuration. Provider acceptance is recorded; delivery/read webhooks are still outside this change. The old statements below describing "everything working" are historical and superseded by this correction.
